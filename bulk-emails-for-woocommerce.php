...
define( 'WPO_BEW_VERSION', '1.0.0' );
define( 'WPO_BEW_PLUGIN_URL', plugin_dir_url( __FILE__ ) );

add_action( 'init', 'wpo_bew_load_textdomain', 10, 1 );
add_filter( 'bulk_actions-edit-shop_order', 'wpo_bew_bulk_actions', 16 );
add_action( 'admin_footer', 'wpo_bew_email_selector' );
add_action( 'wp_ajax_wpo-bew-send-emails', 'wpo_bew_ajax_send_emails' );
add_action( 'wpo_bew_schedule_email_sending', 'wpo_bew_send_order_email' );
add_action( 'admin_notices', 'wpo_bew_admin_notices' );
add_action( 'admin_notices', 'wpo_bew_need_wc' );

function wpo_bew_load_textdomain() {
	load_plugin_textdomain( 'bulk-emails-for-woocommerce', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
}

function wpo_bew_bulk_actions( $actions ) {
	$actions['wpo_bew_send_emails'] = __( 'Send emails', 'bulk-emails-for-woocommerce' );
	return $actions;
}

function wpo_bew_email_selector() {
	global $post_type;

	if ( $post_type == 'shop_order' && get_current_screen()->id == 'edit-shop_order' ) {
		?>
		<div id="wpo_bew_email_selection" style="display:none;">
			<select name="wpo_bew_email_select" style="width:200px;">
		...
		<script>
			jQuery( function ( $ ) {
				$( document ).on( 'change', ".post-type-shop_order select[name='action'], .post-type-shop_order select[name='action2']", function ( e ) {
					...
					if ( actionSelected == 'wpo_bew_send_emails' ) {
						$( '#wpo_bew_email_selection' )
							.show()
						...
						$( '#wpo_bew_email_selection' ).hide().closest( 'body' ).find( '.wp-list-table' ).css( {
							'margin-top': 'initial',
						} );
					}
				} );
				...
				$( document ).on( 'click', '.post-type-shop_order #doaction, .post-type-shop_order #doaction2', function ( e ) {
					...
					if ( action == 'wpo_bew_send_emails' ) {
						...
						let selected_email = $( 'select[name="wpo_bew_email_select"]' ).val();
						...
						$.ajax( {
							...
							success: function( response ) {
								if ( response.success ) {
									window.location.replace( window.location.href + "&wpo_bew=success" );
								} else {
									window.location.replace( window.location.href + "&wpo_bew=error" );
								}
							},
						} );
					}
...
function wpo_bew_ajax_send_emails() {
	...
	foreach ( $order_ids as $order_id ) {
		as_schedule_single_action( strtotime( 'now' ), 'wpo_bew_schedule_email_sending', compact( 'order_id', 'email_to_send' ) );
	}
...
function wpo_bew_send_order_email( $order_id, $email_to_send ) {
...
function wpo_bew_admin_notices() {
	if ( isset( $_REQUEST['wpo_bew'] ) ) {
		$type = sanitize_text_field( $_REQUEST['wpo_bew'] );
		switch ( $type ) {
			case 'error':
